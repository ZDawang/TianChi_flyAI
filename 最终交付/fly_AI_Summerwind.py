                    ...
                    if ef[0][1] != df[0][1]:
                        ...
                        for string in samestartstring:
                            ...
                            if dfstartindex != 0 and (FlightD[df[1][dfstartindex]][1] != 0 and FlightD[df[1][dfstartindex]][1] == FlightD[df[1][dfstartindex - 1]][1]):
                                continue
                            #若不满足中间台风机场不能停机限制
                            if FlightD[ef[1][efstartindex]][4] in sceneportset and not IsTimeSameRange(FlightD[ef[1][efstartindex]][2] + NewdelayD.get(ef[1][efstartindex], datetime.timedelta(minutes = 0)), FlightD[df[1][dfstartindex]][2] + NewdelayD.get(df[1][dfstartindex], datetime.timedelta(minutes = 0)), 0, Scene):
                                continue
                            #计算成本
                            newsubcost = 0
                        ...
                        for string in samestring:
                            ...
                            if dfendindex != ldf and (FlightD[df[1][dfendindex]][1] != 0 and FlightD[df[1][dfendindex]][1] == FlightD[df[1][dfendindex - 1]][1]):
                                continue
                            #若不满足中间台风机场不能停机限制
                            if FlightD[ef[1][efstartindex]][4] in sceneportset and not IsTimeSameRange(FlightD[ef[1][efstartindex]][2] + delayD.get(ef[1][efstartindex], datetime.timedelta(minutes = 0)), FlightD[df[1][dfstartindex]][2] + delayD.get(df[1][dfstartindex], datetime.timedelta(minutes = 0)), 0, Scene):
                                continue
                            if FlightD[ef[1][efendindex - 1]][5] in sceneportset and not IsTimeSameRange(FlightD[ef[1][efendindex - 1]][3] + delayD.get(ef[1][efendindex - 1], datetime.timedelta(minutes = 0)), FlightD[df[1][dfendindex - 1]][3] + delayD.get(df[1][dfendindex - 1], datetime.timedelta(minutes = 0)), 1, Scene):
                                continue
        ...
        while (EFindexlist2 and Up_index == 0):
            ...
            for string in samestring:
                ...
                if ef2endindex != lef2 and (FlightD[ef2[1][ef2endindex]][1] != 0 and FlightD[ef2[1][ef2endindex]][1] == FlightD[ef2[1][ef2endindex - 1]][1]):
                    continue
                #若不满足中间台风机场不能停机限制
                if FlightD[ef[1][efstartindex]][4] in sceneportset and not IsTimeSameRange(FlightD[ef[1][efstartindex]][2] + delayD.get(ef[1][efstartindex], datetime.timedelta(minutes = 0)), FlightD[ef2[1][ef2startindex]][2] + delayD.get(ef2[1][ef2startindex], datetime.timedelta(minutes = 0)), 0, Scene):
                    continue
                if FlightD[ef[1][efendindex - 1]][5] in sceneportset and not IsTimeSameRange(FlightD[ef[1][efendindex - 1]][3] + delayD.get(ef[1][efendindex - 1], datetime.timedelta(minutes = 0)), FlightD[ef2[1][ef2endindex - 1]][3] + delayD.get(ef2[1][ef2endindex - 1], datetime.timedelta(minutes = 0)), 1, Scene):
                    continue
                #构造新解
                NewFlightExecuteList = copy.deepcopy(FlightExecuteList)
                NewFlightDismissList = copy.deepcopy(FlightDismissList)
            ...
            for flightcycle in ef2lightcycle:
                ...
                for k in range(-1, lef):
                    ...
                    #判断台风停机问题
                    if k == -1:
                        if ef[1] and flightcycle_startport in sceneportset and not IsTimeSameRange(FlightD[ef[1][0]][2] + delayD.get(ef[1][0], datetime.timedelta(minutes = 0)), FlightD[flightcycle[-1]][3] + delayD.get(flightcycle[-1], datetime.timedelta(minutes = 0)), 1, Scene):
                            continue
                    elif k == lef - 1:
                        if flightcycle_startport in sceneportset and not IsTimeSameRange(FlightD[ef[1][-1]][3] + delayD.get(ef[1][-1], datetime.timedelta(minutes = 0)), FlightD[flightcycle[0]][2] + delayD.get(flightcycle[0], datetime.timedelta(minutes = 0)), 1, Scene):
                            continue
                    else:
                        if flightcycle_startport in sceneportset and not IsTimeSameRange(FlightD[ef[1][k]][3] + delayD.get(ef[1][k], datetime.timedelta(minutes = 0)), FlightD[flightcycle[0]][2] + delayD.get(flightcycle[0], datetime.timedelta(minutes = 0)), 0, Scene):
                            continue
                        if flightcycle_startport in sceneportset and not IsTimeSameRange(FlightD[ef[1][k + 1]][2] + delayD.get(ef[1][k + 1], datetime.timedelta(minutes = 0)), FlightD[flightcycle[-1]][3] + delayD.get(flightcycle[-1], datetime.timedelta(minutes = 0)), 0, Scene):
                            continue
...
def sortSolution(FlightD, FlightDismissList, delayD, Scene):
    ...
    while(index == 1):
        ...
        for i in range(len(FlightDismissList)):
            ...
            if startport in sceneportset:# and FlightD[df[1][-1]][3] < Scene.ix[0][1]:
                continue
            #寻找可向df后面插的串
            for j in range(len(FlightDismissList)):
                if i == j:
            ...
            for list1 in mat[endport][j]:
                ...
                if df[0][1] == FlightDismissList[list1[0]][0][1]:
                    ...
                    if FlightD[f][2] + delayD.get(f, datetime.timedelta(minutes = 0)) >= endtime:
                        #台风不能停机满足
                        if not (endport in sceneportset) or IsTimeSameRange(endtime, FlightD[f][2] + delayD.get(f, datetime.timedelta(minutes = 0)), 0, Scene):
            ...
            for list2 in mat[j][startport]:
                ...
                if df[0][1] == FlightDismissList[list2[0]][0][1]:
                    ...
                    if FlightD[f][3] + delayD.get(f, datetime.timedelta(minutes = 0)) <= starttime:
                        #台风不能停机满足
                        if not (startport in sceneportset) or IsTimeSameRange(starttime, FlightD[f][3] + delayD.get(f, datetime.timedelta(minutes = 0)), 0, Scene):
        ...
        if FlightD[f][1] == 1 and FlightD[f][8] != 0:
            ...
            if FlightD[f][9] != 0:
                ...
                if FlightD[f][3] > FlightD[jointf][2]:
                    continue
                #确定第一个航班的延迟时间
                if FlightD[f][4] in sceneportset:
                ...
                if FlightD[f][4] in sceneportset:
                    ...
                    if delaytime > datetime.timedelta(hours = 12):
                        continue
    ...
    if mode == 0:
        ...
        while(k < KTimes):
            ...
            if DownRCLSize != 0:
                ...
                DownRCL, UpRcl = GRASPOnce(DownRCL[i][1], DownRCL[i][2], DownRCL[i][0], DownRCL[i][3], FlightD, AirplaneLimit, Scene, num, FlyD, FlyDEnable, DownEnable, delayMaxhour, delayDcompare, AirportCloseD, Israndom)
            else:
                ...
                DownRCL, UpRcl = GRASPOnce(minSolution[0], minSolution[1], mincost, minSolution[2], FlightD, AirplaneLimit, Scene, num, FlyD, FlyDEnable, DownEnable, delayMaxhour, delayDcompare, AirportCloseD, Israndom)
    
    #随机在领域解附近取相同成本的领域解，再寻找下降解
    if mode == 1:      
        k = 0
        num = 0
        while(k < KTimes):
            ...
            if DownRCLSize != 0:
                ...
                DownRCL, UpRcl = GRASPOnce(UpRcl[i][1], UpRcl[i][2], UpRcl[i][0], UpRcl[i][3], FlightD, AirplaneLimit, Scene, num, FlyD, FlyDEnable, DownEnable, delayMaxhour, delayDcompare, AirportCloseD, Israndom)

    #将受影响航班打散，构造机场-机场航班字典，寻找下降解
    if mode == 2:
        k = 0
        FlyDEnable = 1
        while(k < KTimes):
            ...
            if DownRCLSize != 0:
                ...
                print(k, DownRCLSize, mincost)
                DownRCL, UpRcl = GRASPOnce(DownRCL[i][1], splitsolute, DownRCL[i][0], DownRCL[i][3], FlightD, AirplaneLimit, Scene, num, FlyD, FlyDEnable, DownEnable, delayMaxhour, delayDcompare, AirportCloseD, Israndom)
            else:
                if k % 100 == 0:
                    print(2, k)
                if k % period == 0:
                ...
                DownRCL, UpRcl = GRASPOnce(UpRcl[i][1], splitsolute, UpRcl[i][0], UpRcl[i][3], FlightD, AirplaneLimit, Scene, num, FlyD, FlyDEnable, DownEnable, delayMaxhour, delayDcompare, AirportCloseD, Israndom)
    #将受影响航班打散，构造机场-机场航班字典（不同机型可以互换），寻找下降解
    if mode == 3:
        k = 0
        FlyDEnable = 1
        while(k < KTimes):
...
#从Excel表格中获得数据
fileAddr = "D:/python/fly_AI/dd/厦航大赛数据20170705_1.xlsx"
Flight, AirplaneLimitation, AirportClose, Scene, TravelTime = GetOriginData(fileAddr)
#获得一些所需要的集合信息
...
restime1 = time2 - time1
print("总时间为：", restime1.seconds / 60, "分钟")
#使用GRASP进行处理,第一遍获得局部最优
k = 500
p = 30
Israndom = 0
delayMaxhour1 = delayhour
mincost1, minSolution1 = GRASP(FlightExecuteList, FlightDismissList, initialdelayD, initialCost, FlightD, AirplaneLimit, Scene, 0, delayMaxhour1, k, p, delayDcompare, AirportCloseD, Israndom)

time3 = datetime.datetime.now()
restime2 = time3 - time1
print("总时间为：", restime2.seconds / 60, "分钟")
#第二遍，不拆分DF，领域寻找下降解
k = 3000
p = 30
Israndom = 0
delayMaxhour2 = delayhour
m10 = copy.deepcopy(minSolution1[0])
m11 = copy.deepcopy(minSolution1[1])
m12 = copy.deepcopy(minSolution1[2])
mincost2, minSolution2 = GRASP(m10, m11, m12, mincost1, FlightD, AirplaneLimit, Scene, 1, delayMaxhour2, k, p, delayDcompare, AirportCloseD, Israndom)

time4 = datetime.datetime.now()
restime3 = time4 - time1
print("总时间为：", restime3.seconds / 60, "分钟")
#拆分DF，把航程拉直，邻域寻找下降解
k = 3000
p = 30
Israndom = 0
delayMaxhour3 = delayhour
...
mincost3, minSolution3 = GRASP(m20, m21, m22, mincost2, FlightD, AirplaneLimit, Scene, 2, delayMaxhour3, k, p, delayDcompare, AirportCloseD, Israndom)

time5 = datetime.datetime.now()
restime4 = time5 - time1
print("总时间为：", restime4.seconds / 60, "分钟")
#将取消的10h内的航班加入到DF中，邻域寻找下降解
k = 3000
p = 30
Israndom = 0
delayMaxhour4 = delayhour
...
mincost4, minSolution4 = GRASP(m30, DF, delayD, mincost3, FlightD, AirplaneLimit, Scene, 2, delayMaxhour4, k, p, delayDcompare, AirportCloseD, Israndom)

time6 = datetime.datetime.now()
restime5 = time6 - time1
print("总时间为：", restime5.seconds / 60, "分钟")
#允许不同机型
k = 2000
p = 100
Israndom = 0
...
mincost5, minSolution5 = GRASP(m40, m41, m42, mincost4, FlightD, AirplaneLimit, Scene, 3, delayMaxhour5, k, p, delayDcompare, AirportCloseD, Israndom)
...
#统计****************************************************
print("初始解延迟时间为：", hour, "小时")
print("迭代过程中允许的延迟时间为：", delayMaxhour1)
print("最终成本约为：", mincost5)

time7 = datetime.datetime.now()
restime = time7 - time1
print("总时间为：", restime.seconds / 60, "分钟")